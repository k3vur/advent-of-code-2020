...
function inRange(lowerBound, upperBound, x) {
    return Number(x) >= lowerBound && Number(x) <= upperBound;
}

const allowedEyeColors = new Set(['amb', 'blu', 'brn', 'gry', 'grn', 'hzl', 'oth']);

const rules = [
    { key: 'byr', validator: val => val.length === 4 && inRange(1920, 2002, val) },
    { key: 'iyr', validator: val => val.length === 4 && inRange(2010, 2020, val) },
    { key: 'eyr', validator: val => val.length === 4 && inRange(2020, 2030, val) },
    {
        key: 'hgt',
        validator: val => {
            const regexMatch = /^(\d{2,3})(in|cm)$/.exec(val);
            return regexMatch != null &&
                (regexMatch[2] === 'in' && inRange(59, 76, regexMatch[1]) ||
                    regexMatch[2] === 'cm' && inRange(150, 193, regexMatch[1]));
        }
    },
    { key: 'hcl', validator: val => val.match(/^#[0-9a-f]{6}$/) },
    { key: 'ecl', validator: allowedEyeColors.has.bind(allowedEyeColors) },
    { key: 'pid', validator: val => val.match(/^[0-9]{9}$/) }
];

function passPortParagraphToObject(paragraph) {
...
function isValid(passportObject) {
    return rules.every(({key, validator}) => passportObject[key] != null && validator(passportObject[key]));
}

function calculateResult(input) {
    return input.split("\n\n")
        .map(passPortParagraphToObject)
        .filter(isValid)
        .length;
};
