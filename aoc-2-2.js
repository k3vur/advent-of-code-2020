...
function isValid(policy, password) {
	const index1Valid = password.charAt(policy.index1) === policy.char;
	const index2Valid = password.charAt(policy.index2) === policy.char;

	// weird XOR hack (js does not have xor)
...
function parseInput(input) {
	return input.split('\n')
		.map(line => {
			...
			return {
				policy: {
					index1: Number(tokens[0]) - 1,
					index2: Number(tokens[1]) - 1,
					char: tokens[2],
				},
				password: tokens[3]
			};
...
function calculateResult(input) {
	return parseInput(input)
		.filter(parsedLine => isValid(parsedLine.policy, parsedLine.password))
		.length;
}
